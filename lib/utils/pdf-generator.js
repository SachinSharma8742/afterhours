...
export async function downloadTicketPDF(ticket, event) {
  try {
    ...
    const rawDate = event?.start_date || ticket?.start_date;
    const cleanDateStr = typeof rawDate === "string" ? rawDate.replace(" ", "T") : rawDate;
    const parsedDate = cleanDateStr ? new Date(cleanDateStr) : null;
    const displayDate = parsedDate && !isNaN(parsedDate.getTime()) ? parsedDate.toLocaleDateString() : "Upcoming Event";
    doc.text(`Date: ${displayDate}`, 25, 65);
    ...
    doc.roundedRect(55, 92, 100, 100, 4, 4, "F");
    ...
    doc.addImage(qrDataUrl, "PNG", 60, 97, 90, 90);
...
export async function downloadQRImage(ticket) {
  try {
    ...
    const qrDataUrl = await QRCode.toDataURL(qrData, {
      errorCorrectionLevel: "H",
      margin: 2,
      width: 1200, // High resolution for mobile saving & printing
    });
